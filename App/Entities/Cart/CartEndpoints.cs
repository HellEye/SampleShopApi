...
public static class CartEndpointsExtensions {
	public static void MapCartEndpoints(this WebApplication app) {
		...
		cartGroup.MapDelete("/{albumId:int}", async (
		  int albumId,
		  HttpRequest request,
		  HttpResponse response,
		  DeleteFromCartHandler handler
			...
			var res = await handler.Handle(cartId, albumId);
			if (!res.IsSuccess) {
				return res.ToHttpResult();
			}
			response.WithCartIdCookie(res.Value!.Id);

			return Result<Cart>.NoContent().ToHttpResult();
		})
		...
		cartGroup.MapDelete("/", async (
		  ...
		  ClearCartHandler handler
			...
			var res = await handler.Handle(cartId);
			if (!res.IsSuccess) {
				return res.ToHttpResult();
			}
			response.WithCartIdCookie(res.Value!.Id);
